/**
 * This is a list of the NPS activities and their ID's (circa April 15th 2025). Below are reverse lookup functions for name -> activity id and the reverse
 */

export const NPS_ACTIVITY_LIST = [
    {
...
// 🔁 name → id
export const NPS_ACTIVITY_NAME_TO_ID = Object.fromEntries(
    NPS_ACTIVITY_LIST.map(({ name, id }) => [name.toLowerCase(), id])
);

// 🔁 id → name
export const NPS_ACTIVITY_ID_TO_NAME = Object.fromEntries(
    NPS_ACTIVITY_LIST.map(({ name, id }) => [id, name])
);
